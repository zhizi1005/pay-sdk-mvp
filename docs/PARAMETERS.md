...
实例方法见 [`output/SDK.md`](../output/SDK.md)：`ready` / `mount` / `pay` / `openAction` / `getLastTraceId` / `destroy`。
...
在 `onAction` 用 `sdk.getBridge()`（或 `RampPay.getNativeBridge(bridgeName)`）调 `openPayWebUrl` / `openPayChallenge` / `openPayMethod`。`bridgeName` 默认 `'NativeBridge'`，见 [`output/WEBVIEW.md`](../output/WEBVIEW.md)。
